...
data class Player @JvmOverloads constructor(
        ...
        , @ColumnInfo(name = "he_point_sum") var hePointSum: Int = 0                //和了点数之和，用于算平均和了点数
        ...
        , @ColumnInfo(name = "chong_point_sum") var chongPointSum: Int = 0             //放铳点数之和  用来计算平均放铳点数
        , var top: Int = 0                         //一位次数
        , var second: Int = 0                      //二位次数
        , var third: Int = 0                       //三位次数
        , var last: Int = 0                        //四位次数      场均顺位由以上四者平均得
        , var fei: Int = 0                         //被飞次数   注意 被飞不一定是四位
        , var score_sum: Int = 0                   //马点总和   除以局数就是平均马点
    ...
    override fun writeToParcel(dest: Parcel?, flags: Int) {
        ...
        dest?.writeInt(totalDeal)
        dest?.writeInt(tsumo)
        dest?.writeInt(ronn)
        dest?.writeInt(heCount)
        dest?.writeInt(hePointSum)
        dest?.writeInt(hePointMax)
        dest?.writeInt(richiCount)
        dest?.writeInt(richiHe)
        dest?.writeInt(ihhatsuCount)
        dest?.writeInt(chongCount)
        dest?.writeInt(chongPointSum)
        dest?.writeInt(top)
        dest?.writeInt(second)
        dest?.writeInt(third)
        dest?.writeInt(last)
        dest?.writeInt(fei)
        dest?.writeInt(score_sum)
        dest?.writeInt(noManguan)
        dest?.writeInt(manguan)
        dest?.writeInt(tiaoman)
        dest?.writeInt(beiman)
        dest?.writeInt(sanbeiman)
        dest?.writeInt(yakuman)
        dest?.writeInt(score)
    }
